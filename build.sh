...
docker exec -i -t "${PKGBUILD_CONTAINER}" /bin/bash -c 'cd /tmp/vyos-build-arm64 && ./build-packages.sh && cd vyos-build && export VYOS_BUILD_FLAVOUR=generic-arm64 && ./configure --build-by="${BUILD_BY}" --architecture "arm64" && make arm64'
